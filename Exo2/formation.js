...
var Formation = /** @class */ (function () {
    ...
    Formation.prototype.calculerMoyenneFormation = function () {
        if (this.stagiaires.length === 0)
            return 0;
        var sommeMoyennes = this.stagiaires.reduce(function (sum, stagiaire) { return sum + stagiaire.calculermoyenne(); }, 0);
    ...
    Formation.prototype.getNoteMinStagiaire = function (nom) {
        var stagiaire = this._stagiaires.find(function (s) { return s._nom === nom; });
        if (stagiaire) {
    ...
    Formation.prototype.afficherNomMax = function () {
        ...
        if (indexMax !== -1) {
            console.log("Le meilleur stagiaire est : ".concat(this._stagiaires[indexMax]._nom));
        }
        else {
    ...
    Formation.prototype.afficherMinMax = function () {
        ...
        if (indexMax !== -1) {
            ...
            console.log("La note minimale du meilleur stagiaire est : ".concat(stagiaireMax.trouvermin()));
        }
        else {
            console.log("Aucun stagiaire dans la formation");
        }
    ...
    Formation.prototype.trouverMoyenneParNom = function (nom) {
        var stagiaire = this._stagiaires.find(function (s) { return s._nom === nom; });
        if (stagiaire) {
            console.log("Moyenne pour ".concat(nom, ": ").concat(stagiaire.calculermoyenne().toFixed(2)));
        }
        else {
